...
RUN_ARGS = --rm -v "$(shell pwd):/src:rw" -v "/etc/passwd:/etc/passwd:ro" -v "/etc/group:/etc/group:ro" \
           --workdir "/src" -u "$(shell id -u):$(shell id -g)"
RUN_INTERACTIVE ?= --tty --interactive

.PHONY : help install-php test-php shell-php install test
.SILENT : help install-php shell-php test
.DEFAULT_GOAL : help
...
install-php: ## Install all php dependencies
...
shell-php: ## Start shell into container with php
...
test: test-php ## Execute all dependencies

shell: shell-php ## Start shell into default container
