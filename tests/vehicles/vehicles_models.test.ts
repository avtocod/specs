...
const expected_items_count: {[key: string]: number} = {
    'models.json': 3105,
...
describe.each(groups_list)('%s group of vehicles specs', group_name => {
    ...
    describe.each(models_file_names)('%s file', models_file_name => {
        ...
        test.concurrent(`has ${expected_items_count[models_file_name]} items`, async () => {
            expect(models.length).toBe(expected_items_count[models_file_name]);
        });
