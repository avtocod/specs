...
import { VehicleTransmissionType } from './vehicles';

const file_name = 'transmission_types.json';
...
describe.each(groups_list)(file_name + ' file in %s group of vehicle specs', group_name => {
    ...
    const items: Array<VehicleTransmissionType> = require(specs_path);
    ...
    describe.each(items)('item %j has', (item) => {
        ...
        test.concurrent(`"id" property with value matching with pattern "^ID_TRANSMISSION_TYPE_.*$`, async () => {
            expect(item.id).toMatch(new RegExp('^ID_TRANSMISSION_TYPE_.*$'));
        });
